...
Basalt의 프롬프트는 고정 문장 하나가 아니라 태스크 상태와 대상 프로젝트에 맞춰 조립됩니다.

```text
Input:
- 사용자 요청: 제목, 설명, 명확화 답변, 스펙 확장
- 프로젝트 문맥: 파일 구조, package.json, 라우터, 설치 패키지, UI 컴포넌트
- 실행 문맥: 이전 step 로그, 실패 원인, QA 결과, 변경 파일
- 제약: import 가능 범위, RSC/metadata 규칙, 상태 전이, 위험 skill 정책

Output:
- plan 단계: workflow, required agents, risk, next action
- execute 단계: 코드 변경, 로그, fileChanges, repair 후보
- verify 단계: pass/fail, QA 메타데이터, 후속 조치
```

주요 LLM 호출 목적은 다음과 같습니다.

| 호출/스킬 | 목적 | 주로 쓰는 결과 |
| --- | --- | --- |
| `analyze_task` | 요청 성격, 필요한 역할, 위험 신호 분석 | required agents, risk |
| `create_workflow` | 실행 가능한 step 목록 생성 | `Tasks.workflow` |
| `consult_agents` | 역할별 agent 의견과 handoff 맥락 생성 | plan 보강, 실행 로그 |
| `write_code` | 프로젝트 문맥과 제약을 반영한 코드 생성/수정 | 변경 파일, diff, repair 대상 |
| `verify_final_output` | 결과가 요청과 검증 조건을 만족하는지 판단 | verification metadata |
| `recovery-suggestions` | 실패/QA 정보를 다음 시도용 가이드로 압축 | 복구 Markdown |
| `handoff-summary` | 실행 기록을 팀 공유용 문서로 압축 | 인수인계 Markdown |

...
- Dev 종료 ~ Test 진입
  - 워크플로 완료 후 `testing` 전 **`runDevExitQaPipeline`**: 대상 dev URL에 **`runQaPageSmokeCheck`**. 스모크 실패 시 **`maybeScaffoldMinimalUiKit`**(`lib/project-ui-kit.ts`)로 `components/ui` 경로를 **`lib/tsconfig-paths.ts`**의 병합 `paths`(`@/*`)와 맞춰 **비-LLM 갭 필**(button/input/label·배럴 등)을 먼저 시도하고, 새 파일이 생기면 해당 라운드에서는 LLM 자동 수정을 건너뛴 뒤 재스모크한다. 여전히 실패하면 `write_code` 자동 수정·캡처·`metadata.qaSignoff` 흐름은 기존과 동일. 구현: `lib/qa/page-smoke-check.ts`, `lib/project-ui-kit.ts`, `lib/tsconfig-paths.ts`.
- 잠금

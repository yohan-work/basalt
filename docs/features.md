...
- **`verify_final_output`**(`lib/skills/verify_final_output/execute.ts`)는 LLM 판단 외에, `metadata.fileChanges`에 있는 App Router **`app/**/page.*`(및 `src/app/...`)** 경로에 대해 **결정적 sanity 검사**를 수행합니다(`lib/qa/app-route-page-sanity.ts`). 예: 기본보내기가 배열·객체 리터럴이거나 mock 전용 바인딩이면 `verified: false`로 **강제**합니다. 또한 루트 **`app/layout.*` / `src/app/layout.*`**에 `<html>`·`<body>` 누락이 있으면 `lib/qa/app-root-layout-sanity.ts` 휴리스틱으로 실패 처리합니다([missing root layout tags](https://nextjs.org/docs/messages/missing-root-layout-tags)). 프롬프트에는 루트 한 단계 목록 외에 **깊이 제한 디렉터리 트리 샘플**과 **변경 파일 내용 스니펫**이 포함됩니다.
...
- **진행·스코프 파일(블로그식 progress와 유사)**: 대상 프로젝트 `.basalt/ralph/<taskId>/progress.md`에 라운드마다 태스크 상태·`metadata.fileChanges` 경로 요약 등이 append되며, 다음 라운드 `plan` 입력에 **진행 로그**로 포함됩니다. 같은 디렉터리에 `scope.md` 또는 `prd.json`을 두면(선택) 플랜 앞에 **범위/PRD**로 병합됩니다.
- **피드백 게이트(옵트인)**: `BASALT_RALPH_FEEDBACK_GATE=true`(또는 `1`/`yes`)이면 `execute`가 `testing`으로 끝난 직후 `verify` 전에, 대상 `package.json`에 `typecheck` / `test` / `lint` 스크립트가 있으면 그 순서로 `npm run …`을 실행합니다. 실패 시 출력 일부가 가드레일에 남고 태스크는 `failed`(`failedAction`: `ralph_feedback_gate`)로 표시된 뒤 다음 라운드로 이어집니다. 타임아웃은 `BASALT_RALPH_FEEDBACK_GATE_TIMEOUT_MS`(밀리초, 기본 420000)로 조정할 수 있습니다.
- **HITL 한 번만**: `BASALT_RALPH_MAX_ROUNDS=1`이면 단일 라운드만 도는 “ralph-once”에 가깝게 쓸 수 있습니다.
...
- **프로젝트 전체 typecheck (write_code 배치 직후)**: 한 스텝에서 생성된 **모든** 파일을 디스크에 쓴 뒤(정렬상 `components/ui` 등이 먼저 기록됨), 단일 파일·경계 검증을 통과했다면 `lib/qa/project-typecheck.ts`가 대상 루트에서 **`npm run typecheck`**(있으면) 또는 **`npx tsc --noEmit -p tsconfig.json` / `jsconfig.json`**을 실행해 **교차 파일** 타입 오류를 잡는다. 실패 시 기존과 동일한 **`write_code_typescript_repair`**로 진단 로그를 넘겨 한 파일씩 고치고 다시 쓴다(최대 5라운드). 끄려면 **`BASALT_SKIP_PROJECT_TYPECHECK=1`**. 타임아웃은 **`BASALT_PROJECT_TYPECHECK_TIMEOUT_MS`**(밀리초, 기본 120000). 대상에 `typecheck` 스크립트도 `tsconfig`/`jsconfig`도 없으면 **건너뜀**. 완전한 무결은 의존성·LLM 한계로 보장할 수 없고, **Ralph 피드백 게이트**(`BASALT_RALPH_FEEDBACK_GATE`, §5d)와는 별개로 **실행 중**에 조기에 맞추는 용도다.

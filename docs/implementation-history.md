...
- 오케스트레이터의 `filesystemSkills`에 `apply_design_system`, `generate_scss`를 넣어 **`projectPath`가 마지막 인자로 붙는다** (`read_codebase`와 동일 패턴).
...
- `lib/agents/Orchestrator.ts` (`filesystemSkills`)

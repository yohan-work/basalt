...
- **Phase C (스킬 인자 Zod 검증)**: `lib/skills/arg-schemas.ts`의 `validateSkillArgsBeforeExecution`이 오케스트레이터 일반 스킬 경로에서 `generateSkillArguments` 직후 원시 인자를 검사한다. 레지스트리 이름 정규화는 `normalizeSkillRegistryName`으로 `getSkillRegistryEntry`와 맞춘다. 1차 스키마는 `read_codebase`, `list_directory`, `run_shell_command`, `manage_git`. 단위 테스트는 `npm run test:arg-schemas`(`lib/skills/arg-schemas.test.ts`, devDependency `tsx`).

**참조 파일**

- `lib/prompts/`, `lib/llm.ts`, `lib/codegen/multi-phase-write-code.ts`, `lib/agents/Orchestrator.ts`, `lib/skills/registry.ts`, `lib/skills/arg-schemas.ts`, `lib/stream-emitter.ts`

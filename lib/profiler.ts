...
- **Preferred order:** (1) Use semantic HTML + existing project styles. (2) Or add primitives with \`write_code\` **before** pages that need them. (3) For React/Next targets, Basalt may auto-create minimal \`button\`/\`input\`/\`label\` at execute start — see task metadata \`uiKitScaffold.files\` when present; after that, \`USE_EXISTING\` applies on the next context refresh.`;
        ...
        const tanstackTableDepHint = data.dependencies.includes('@tanstack/react-table')
            ? '\n- **TanStack Table (v8)**: Use `useReactTable` + `getCoreRowModel` — **not** `useTable`. Import `ColumnDef`, `flexRender`, `useReactTable`, row models **only** from `@tanstack/react-table` (every file that calls `flexRender` must import it — avoids **TS2552**). Import `Table`, `TableRow`, `TableCell`, `TableHead`, `TableHeader`, `TableBody`, etc. **only** from `@/components/ui/table` — **never** `flexRender` from UI. **Header API**: use `header.column.columnDef` — **never** `header.columnDef` (**TS2551**). For **TS7006** on `cell`/`header` in `.map`, use `import type { Cell, Header } from \'@tanstack/react-table\'` and e.g. `(cell: Cell<YourRow, unknown>)` or type `useReactTable<YourRow>(…)`. For column width use `size` / `minSize` / `maxSize` on the column def and `header.getSize()` / `column.getSize()` — do not use `meta.width` unless you extend `ColumnMeta` via `declare module \'@tanstack/react-table\'` (**TS2339**). Type `headerGroup` / `row` / `cell` in `.map` callbacks to avoid implicit `any`. **Rules of Hooks**: call `useReactTable` and `useMemo` for `columns` **before** any `if (loading) return …`; use `data: rows ?? []` and handle loading in JSX after all hooks — see https://react.dev/reference/rules/rules-of-components-and-hooks . Intro: https://tanstack.com/table/latest/docs/introduction · sizing: https://tanstack.com/table/latest/docs/guide/column-sizing . **Minimal `Button`** (Basalt scaffold): **no** `asChild` — use `<Link className="...">` or `<button>`.'
            : '';

        const tableComponentHint = data.availableUIComponents.includes('table')
            ? '\n- **TanStack Table + UI**: When `table` is on disk, extended scaffold exports full shadcn-style names from `@/components/ui/table`. Still: logic only from `@tanstack/react-table`; always `flexRender` for header/cell; narrow `accessorKey`; avoid `asChild` on minimal `Button`. Call `useReactTable` / `useMemo(columns)` **before** any loading early-`return` (`data: rows ?? []`).'
            : '';

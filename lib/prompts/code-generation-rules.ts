...
- **DESIGN PRESET OVERRIDE (CRITICAL)**: If \`[PROJECT CONTEXT]\` / \`DESIGN HINTS\` includes \`## DESIGN PRESET\`, and the task is generating a **new page**, that preset overrides the default visual fallback below.
  - In that case, do **not** use the dark-first baseline unless the target repo already forces a dark product shell or the active preset itself is dark/monochrome.
  - Follow the active preset's tone summary and markdown guidance for whitespace, palette, typography rhythm, surface treatment, and motion.
  - **Apple**: bright premium surfaces, restrained monochrome palette, generous whitespace, cinematic calm.
  - **Vercel**: black-and-white precision, sharp contrast, developer-product minimalism, compact chrome.
  - **Notion**: warm minimalism, editorial structure, soft surfaces, calm document-like hierarchy.
  - Avoid mixing preset languages. Pick one coherent aesthetic and carry it through the page.
...
- **EXECUTION UI (Request Work / any target repo)**: **First** match the **target** project's visual language from [PROJECT CONTEXT], **DESIGN HINTS**, and files you read — reuse CSS variables, Tailwind theme tokens, and component patterns when they clearly define the product. If a **DESIGN PRESET** is active for the task, follow that preset before falling back to the default tone. **When no such established style exists** and the task does not override, use **DEFAULT VISUAL TONE** so output stays cohesive instead of random palettes. Do **not** fight existing tokens with unrelated hex colors; prefer neutral spacing and accessible contrast.

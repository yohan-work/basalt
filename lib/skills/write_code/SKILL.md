...
**Boards / data tables (TanStack v8)**: Use **`useReactTable`** + **`getCoreRowModel`** from `@tanstack/react-table` — **not** `useTable`. Import **`flexRender`**, **`ColumnDef`**, row models **only** from `@tanstack/react-table`; import **`Table`**, **`TableRow`**, **`TableCell`**, etc. **only** from `@/components/ui/table`. Type **`headerGroup` / `row` / `cell`** in `.map` callbacks (avoid implicit `any`). Basalt extended scaffold creates a full **`table.tsx`** when that import was missing. **Minimal `Button`**: do **not** use **`asChild`** — use `<Link className="...">` or `<button>`.

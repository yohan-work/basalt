...
8.  **CRITICAL (Next.js App Router)**:
    ...
    - For other files under `app/` (e.g. leaf components not exporting metadata), if you use React hooks or event handlers that require a Client Component, include `"use client"` as the first statement (after comments only). Basalt `write_code` rejects invalid metadata + `"use client"` combinations before writing.
...
13. **PRISMA / DB CLIENT (when `@prisma/client` is installed)**: In Route Handlers (`app/.../route.ts`), Server Actions, and other server modules, **never** use `prisma.someModel` without defining `prisma` in that file: import the project singleton (e.g. `import { prisma } from '@/lib/prisma'`) or use `import { PrismaClient } from '@prisma/client'` plus `const prisma = new PrismaClient()` when no shared module exists. Match paths to the target repo (`read_codebase`). Bare `prisma` causes **TS2304** and failed writes.

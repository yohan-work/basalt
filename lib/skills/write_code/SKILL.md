...
13. **PLACEHOLDER / DEMO IMAGE URLS (CRITICAL)**: If the task does **not** paste an exact image URL, every hard-coded `http(s)` image used for UI (landing, `/features`, heroes, cards, galleries) MUST be `https://dummyimage.com/<W>x<H>/000/fff` with **only** `<W>` and `<H>` changed. Do **not** use Unsplash, Picsum, via.placeholder.com, placehold.co, etc. Prefer `<img src="...">` for dummyimage to avoid `next/image` remote config.

...
4. **Root layout** (`app/layout.*` or `src/app/layout.*`): must include `<html>` and `<body>` wrapping `{children}` ([missing root layout tags](https://nextjs.org/docs/messages/missing-root-layout-tags)). If snippets or structure show only `return children` without these tags at the root, set `verified` to **false**.
5. Provide a clear reasoning for your verification status. When failing, prefer actionable `suggestedFix` text the implementer can follow in one pass.

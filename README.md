...
## ENV Process
<img width="1723" height="909" alt="image" src="https://github.com/user-attachments/assets/eaa58c0e-3263-43c2-ae04-6a240e37b784" />
...
Basalt는 작은 UI 수정 요청이 자주 들어오는 프론트엔드 개발자와 팀을 주요 사용자로 둡니다. 사용자는 프로젝트를 연결하고 태스크를 만든 뒤, AI가 만든 작업 계획, 실행 로그, 코드 변경 결과, 검증 결과, 후속 정리 내용을 한 화면 흐름에서 확인할 수 있습니다.
...
Basalt는 태스크를 다음 순서로 처리합니다.

1. `ProjectProfiler`가 대상 프로젝트의 파일 구조, package.json, 라우터, UI 컴포넌트, 설치 패키지를 읽습니다.
2. AI가 사용자 요청과 프로젝트 문맥을 바탕으로 작업 계획과 workflow step을 만듭니다.
3. Orchestrator가 step별 agent와 skill을 선택하고, 코드 읽기·쓰기·검토·검증 작업을 순서대로 실행합니다.
4. 코드 생성 단계에서는 라우터 규칙, 설치된 패키지, import 가능 경로, RSC/metadata 제약을 함께 넣어 잘못된 출력을 줄입니다.
5. 실행 후에는 QA smoke, verify, screenshot/responsive 확인 결과를 저장하고, 실패 신호가 있으면 repair 또는 복구 가이드로 연결합니다.

프롬프트에는 사용자 요청, 명확화 답변, 스펙 확장 내용, 프로젝트 구조, 설치된 패키지, 라우터 규칙, 이전 실행 로그, 검증 결과가 함께 들어갑니다. 그래서 AI는 일반적인 코드 답변이 아니라 연결된 프로젝트의 실제 문맥 안에서 계획과 수정 방향을 생성합니다.
...
동작 여부는 다음 결과로 확인할 수 있습니다.

- `Tasks.workflow`: AI가 생성한 작업 계획
- `Execution_Logs`: step별 agent/skill 실행 로그
- `metadata.fileChanges`: 코드 변경 파일과 변경 이력
- `metadata.qaPageCheck`: 대상 앱 페이지 스모크 결과
- `metadata.qaSignoff`: 검수 완료 탭에 표시되는 최종 검증 문구와 캡처 정보
- `.basalt/basalt-qa/<taskId>/`: 대상 프로젝트에 저장되는 QA 캡처 PNG
- 복구 가이드와 인수인계 Markdown: 실패 후 다음 시도 또는 팀 공유에 쓰는 정리 결과
